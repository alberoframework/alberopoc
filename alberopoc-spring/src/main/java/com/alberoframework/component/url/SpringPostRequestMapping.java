...
class SpringPostRequestMapping<R extends Request<?>> extends SpringRequestMapping<R>{
    ...
    @Override
    public R createRequest(HttpServletRequest request) {
        ...
        return BaseBeanUtils.populate(api, pathParams);
    }

    private R createFromMultipart(Class<R> type, HttpServletRequest request) {
        final R api = BaseBeanUtils.newInstance(type);
        ...
        items.forEach(item -> {
            ...
            if (item.isFormField()) {
                BaseBeanUtils.setProperty(api, item.getFieldName(), item.getString());
            } else {
                try {
                    BaseBeanUtils.setProperty(api, item.getFieldName(), new BinaryContent(item.get(), item.getContentType()));
                } catch (Exception e) {
    ...
    private R createWithJson(Class<R> type, HttpServletRequest httpRequest) {
        ...
        return BaseBeanUtils.newInstance(type);
    }
