...
@Component
public class ProjectAppCollectionQueryHandler extends AbstractIssueTrackerEnvelopeQueryHandler<ProjectAppCollectionQuery, HypermediaObjectCollectionResource<ProjectEntity>> {
    @Override
    protected HypermediaObjectCollectionResource<ProjectEntity> doHandle(SimpleAuthenticatedRequestEnvelope<ProjectAppCollectionQuery, HypermediaObjectCollectionResource<ProjectEntity>> env, ContextualizedQueryGateway queryGateway) {
        return Iterables.stream(queryGateway.handle(new ProjectCollectionQuery()))
			            ...
			            .withLink("createProjectCommand", new CreateProjectCommand());
    }
}
